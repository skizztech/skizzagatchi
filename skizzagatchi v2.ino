...
void updateStars(unsigned long dt) {
  ...
  for (int i = 0; i < STAR_COUNT; i++) {
    if (starLife[i] > 0) {
      ...
      if (random(0, 12) == 0)
        starBright[i] = !starBright[i];

      if (starLife[i] > 0 && starBright[i])
...
void drawSleeping() {
  ...
  // Z's moved left (from 55 → 51)
  display.drawBitmap(51, bubbleY - 2,  zBitmap, 5, 7, SSD1306_WHITE);
  display.drawBitmap(51, bubbleY - 10, zBitmap, 5, 7, SSD1306_WHITE);
  display.drawBitmap(51, bubbleY - 18, zBitmap, 5, 7, SSD1306_WHITE);

  // Z drifting upward ONLY
